...
static AX_VOID *IVPS_GetFrameThread(AX_VOID *pArg)
{
    AX_S32 ret = 0;
    SAMPLE_IVPS_GRP_T *pThis = (SAMPLE_IVPS_GRP_T *)pArg;
    AX_VIDEO_FRAME_T engine_input_frame = {0}, chn0_frame = {0};
    ...
    while (!ThreadLoopStateGet())
    {
        ...
        ret = AX_IVPS_GetChnFrame(nGrp, 0, &chn0_frame, 200);
        if (ret != 0)
        {
            printf("AX_IVPS_GetChnFrame(Grp:%d Chn:%d) failed, ret=0x%x.\n", nGrp, 0, ret);
            AX_IVPS_ReleaseChnFrame(nGrp, infer_chn, &engine_input_frame);
            continue;
        }
        ...
        chn0_frame.u64VirAddr[0] = (AX_U64)AX_POOL_GetBlockVirAddr(chn0_frame.u32BlkId[0]);
        chn0_frame.u64PhyAddr[0] = AX_POOL_Handle2PhysAddr(chn0_frame.u32BlkId[0]);

        ret = SAMPLE_ENGINE_Inference(&engine_input_frame, &chn0_frame);
        if (ret != 0)
        {
        ...
        // Send to venc
        AX_VIDEO_FRAME_INFO_T frame_info = {0};
        memcpy(&frame_info.stVFrame, &chn0_frame, sizeof(AX_VIDEO_FRAME_T));
        ret = AX_VENC_SendFrame(0, &frame_info, 200);
        if (ret != 0)
        {
            printf("Send frame to venc %d failed! ret=0x%x\n", 0, ret);
        }
        ...
        ret = AX_IVPS_ReleaseChnFrame(nGrp, 0, &chn0_frame);
        if (ret != 0)
        {
            printf("AX_IVPS_ReleaseChnFrame(Grp:%d Chn:%d) failed, ret=0x%x.\n", nGrp, 0, ret);
        }
